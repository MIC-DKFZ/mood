...
class AE2D:
    ...
    def train(self):
        ...
        val_loader = get_numpy2d_dataset(
            base_dir="/fast/moody/brain/train", num_processes=8, pin_memory=True, batch_size=self.batch_size, mode="val"
        )
    ...
    def score_sample(self, np_array):
        ...
        for i in range(ceil(orig_shape[0] / self.batch_size)):
            ...
            with torch.no_grad():
                ...
                loss = torch.mean(torch.pow(batch - batch_rec, 2), dim=(1, 2, 3))
